...
class Index extends Base
{
	...
	{
		//最新用户
		$user = Db::name('user')->field('id,username,nickname,create_time')->order('id desc')->paginate(8)->each(function ($item) {
			return $item;
		});
		//最新主机
		$server = Db::name('server')->field('id,uid,time,ip_address,hostname')->order('id desc')->paginate(8)->each(function ($item) {
			return $item;
		});
		//统计
		$data = [
	...
	//添加主机
	public function server_add(): string
	{
	...
	//主机列表
	public function server_del(): string
	{
		return $this->curl()->send($this->config['vultr_api'] . '/v1/server/destroy', 'POST', $this->config['vultr_keys'], ['SUBID' => $this->params['sub_id']]);
	}
}