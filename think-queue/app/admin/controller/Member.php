...
class Member extends Base{
	...
	public function index(){
		...
		if(isset($this->params['keywords']) && $this->params['keywords']){
			//关键字搜索
			$list=Db::name('user')->where($where)->where('username|nickname|realname','like','%'.$this->params['keywords'].'%')->order('id desc')->paginate($pageSize,false,['query'=>$this->params]);
		}else{
			//列表
	...
	public function open(){
		...
		Db::startTrans();
		try{
			Db::name('user')->where(['id'=>$this->params['id']])->update(['status'=>1,'lock_uid'=>1,'lock_time'=>$this->timestamp(10)]);
			Db::commit();
			$this->logs()->database(0,2,$this->request->ip(),'恢复【'.$user['username'].'】',$this->admin['id'],0);
		}catch(\Exception $e){
			Db::rollback();
	...
	public function close(){
		...
		try{
			...
			Db::commit();
			$this->logs()->database(1,2,$this->request->ip(),'禁用【'.$user['username'].'】',$this->admin['id'],0);
		}catch(\Exception $e){
			Db::rollback();
			$this->result(0,'操作失败，请重试!');
		}
	...
	public function detail(){
		if($this->request->isPost()){
			...
			try{
				...
				Db::commit();
				//记录日志
				$this->logs()->database(1,2,$this->request->ip(),'修改【'.$user['username'].'】资料',$this->admin['id'],0);
			}catch(\Exception $e){
				Db::rollback();
				$this->error('修改失败，请重试');
			}
